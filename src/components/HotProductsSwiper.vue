<template>
  ...
  >
    <swiper-slide v-for="product in hotProducts" :key="product.id">
      <a
        href=""
        class="card bg-card-bg border-0 h-100 pb-5 rounded-3 position-relative text-dark"
        @click.prevent="handleClick($event, product.id)"
        @mouseenter="iconUp"
        @mouseleave="iconDown"
      >
        <img class="icon-position" src="../assets/images/Product.svg" alt="藥丸icon" />
        <div
        ...
        <div class="card-body px-8 px-sm-5 d-flex flex-column">
          ...
          <div class="mb-4">
            <p class="fw-semibold text-primary-200 text-center">{{ `NT$${product.price} /顆` }}</p>
          </div>
  ...
  computed: {
    ...
    hotProducts() {
      return this.productsAll.filter(
        (product) => product.category === '維生素' || product.category === '功能性補給'
      )
    }
  },
  methods: {
